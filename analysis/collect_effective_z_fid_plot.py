...
for run in all_runs:
    ...
    try:
        with open(os.path.join(run, "test_metric/fid.json"), "r") as f:
...
plt.title
plt.tight_layout()
plt.savefig(savedir+'/fid_vs_dim_effective_z_{}_ld{}.pdf'.format(DATASET,_DIMS[0]), bbox_inches='tight')
